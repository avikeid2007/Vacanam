...
- ⚡ **Standalone Voice Action Commands & Shortcuts** — Control your PC hands-free by speaking commands: `"select all"` (`Ctrl+A`), `"undo that"` (`Ctrl+Z`), `"redo that"` (`Ctrl+Y`), `"copy that"`, `"paste that"`, `"save file"` (`Ctrl+S`), `"press enter"`, `"press tab"`, `"press escape"`, `"delete line"`, `"delete word"`, `"switch window"` (`Alt+Tab`), `"close window"` (`Alt+F4`), and `"lock computer"`.
- ✍️ **Smart Verbal Punctuation & Emoji Formatting** — Dictate natural spoken punctuation (*"comma"*, *"period"*, *"question mark"*, *"new line"*, *"new paragraph"*, *"open paren"*, *"smiley face"*, *"thumbs up"*, *"fire emoji"*) and watch them seamlessly format with proper spacing and auto-capitalization.
- 📝 **Custom Voice Snippets & Expansion Macros** — Define custom trigger phrases (e.g. `"insert signature"`, `"insert date"`, `"my meeting link"`) with dynamic `{DATE}`, `{TIME}`, and `{DATETIME}` tags to expand boilerplate text instantly.
